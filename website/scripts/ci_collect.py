#!/usr/bin/env python3
"""Validate, unpack per-crate tarballs and commit each one individually.
...
After all tarballs: a single git push.
...
def validate_tarball(path: Path) -> tuple[str, str] | None:
    ...
    if len(first_parts) < 2:
        print(f"  INVALID {path.name}: top-level path {members[0]!r} has no "
              f"crate/version prefix", file=sys.stderr)
        return None
    ...
    for m in members:
        if m != prefix and not m.startswith(prefix + "/"):
            print(f"  INVALID {path.name}: unexpected path {m!r} "
                  f"(expected prefix {prefix!r})", file=sys.stderr)
            return None

    # Must document either success or failure
    member_set = set(members)
    if (f"{prefix}/index.json" not in member_set
            and f"{prefix}/error.json" not in member_set):
        print(f"  INVALID {path.name}: contains neither index.json nor error.json",
              file=sys.stderr)
        return None
...
def write_collect_error(data_dir: Path, crate: str, version: str,
                        message: str) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--artifacts", required=True, type=Path, metavar="DIR",
    ...
    for tarball in tarballs:
        ...
        if result is None:
            ...
            parts = stem.rsplit("-", 1)
            if len(parts) == 2:
                write_collect_error(args.data_dir, parts[0], parts[1],
